...
function websocket (env, server, entries, treatments, profiles, pump) {
"use strict";
// CONSTANTS
    ...
    entries.list(q, function (err, results) {
      ...
      treatments.list(function (err, results) {
          ...
          pumpData = results.map(function(pumpRecord) {
              var timestamp = new Date(pumpRecord.timestamp);
              return pumpRecord;
          });
      });
    });
...
function loadData() {
    ...
    if (pumpData) {
        pumpRecord = pumpData.slice();
        pumpRecord.sort(function(a, b) {
            return a.x - b.x;
        });
    }
    ...
    if (actualLength > 1) {
        ...
        var shouldEmit = is_different(actual, predicted, mbg, treatment, errorCode, profile, pump, cal);
        patientData = [actual, predicted, mbg, treatment, errorCode, profile, pump, cal];
